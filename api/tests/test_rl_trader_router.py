"""rl-trader router tests — offline, deterministic (synthetic GBM-regime path).

The whole point of the tool is the honest NULL: on the seeded synthetic price path
(a GBM with mild regime-switching drift/vol + microstructure noise) there is, BY
CONSTRUCTION, no exploitable timing edge net of costs, so the PPO agent does NOT
reliably beat buy-and-hold out-of-sample — ``rl_beats_baseline`` must read FALSE
once the seed lottery + Deflated Sharpe corrections are applied.
...
def _isolate_global_numpy_rng() -> Iterator[None]:
    ...
    The synthetic backtest path advances the process-wide ``np.random`` legacy
...
def test_run_endpoint_synthetic_returns_200(client) -> None:
    """data_source_pref='synthetic' → 200, rl_beats_baseline=False, finite scalars, figures."""
    payload = {
        "n_seeds": 5,
        "cost_bps": 5.0,
        "lookback": 32,
    ...
    resp = client.post("/tools/rl-trader/run", json=payload)
    assert resp.status_code == 200, resp.text
    body = resp.json()

    # --- shape ---
    assert {"summary", "equity_figure", "seed_lottery_figure", "data_source"} <= body.keys()
    assert body["data_source"] == "synthetic"

    summary = body["summary"]
    assert summary["data_source"] == "synthetic"

    # --- THE honest NULL: rl_beats_baseline must be False on the synthetic path ---
    assert summary["rl_beats_baseline"] is False
    ...
    assert 0.0 <= float(summary["dm_pvalue_vs_buyhold"]) <= 1.0
    ...
    for fig_key in ("equity_figure", "seed_lottery_figure"):
        fig = body[fig_key]
...
def test_run_endpoint_minimal_request_returns_200(client) -> None:
    ...
    assert summary["rl_beats_baseline"] is False
    assert math.isfinite(float(summary["oos_sharpe_buyhold"]))
