...
the IN-SAMPLE best configuration under-perform the MEDIAN out-of-sample? A high
PBO (``>= 0.5``) means the selection procedure is more likely than not picking an
in-sample-overfit artifact rather than a genuinely-best out-of-sample
configuration.

THE PROCEDURE (Bailey et al. 2017):

1. take a ``(T, N)`` matrix of per-bar performance (e.g. per-bar net returns) for
...
Ported from ``algo-system:evaluation/pbo.py`` (the sole full implementation in
the portfolio). The PBO feeds the edge verdict: where a grid was selected, an
edge claim requires ``pbo < 0.5``. Importing this module has no side effects.
"""
...
from quantcore._typing import FloatArray
...
def _block_sharpe(block: FloatArray) -> FloatArray:
    ...
    is used directly. A column with zero (or numerically-zero) dispersion has an
    undefined Sharpe; ``-inf`` is substituted so a flat configuration can never be
    the IS-best and always ranks at the OOS bottom (the conservative,
    overfit-leaning choice).
    """
...
def probability_of_backtest_overfitting(
    ...
    *,
    ...
    ``T`` bars, splits the bars into ``n_splits`` contiguous equal blocks, forms
    all :math:`\binom{S}{S/2}` symmetric in-sample/out-of-sample partitions, and
    for each partition finds the IS-best configuration (highest in-sample Sharpe),
    maps its out-of-sample rank to a relative rank ``omega`` and a logit
    ``lambda = ln(omega / (1 - omega))``. The PBO is the fraction of partitions
    with ``lambda <= 0`` — the IS-best config landed in the bottom OOS half.
    ...
    performance:
        ...
        ``n_splits`` non-empty blocks (each IS/OOS half ``>= 2`` rows).
    n_splits:
    ...
    # Split the T bars into S contiguous, (near-)equal blocks. Each IS/OOS half is
    # S/2 blocks, so the smallest half must carry >= 2 rows for the ddof=1 Sharpe.
    ...
    for is_blocks in itertools.combinations(block_indices, half):
        ...
        # IS-best configuration; ties resolve to the lowest index via argmax.
        n_star = int(np.argmax(is_sharpe))

        # OOS average-tie 1-based rank of the IS-best config; relative rank
        # omega = rank / (N + 1) lies strictly in (0, 1), so the logit is finite.
    ...
    # PBO = fraction of partitions whose IS-best config landed in the bottom OOS
    # half (lambda <= 0, i.e. omega <= 0.5 — the median or below).
