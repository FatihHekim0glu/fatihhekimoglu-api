...
def deflated_sharpe_ratio(
    ...
    *,
    ...
    grid (#graph-types x #models x #HP configs — e.g. corr-kNN/sector/empty x
    naive/ridge/gcn/graphsage x the HP grid). The PSR uses the FULL ``(\gamma_4)``
    kurtosis term. The DSR is non-increasing in ``n_trials`` (monotonicity
