...
validated against ``scipy.stats.norm`` in the parity suite). Rather than maintain
a second copy that can drift, these public names are thin wrappers over quantcore
that translate ``quantcore.ValidationError`` into this package's own
:class:`gnnstocks._exceptions.ValidationError` with IDENTICAL messages (the two
packages have no shared exception ancestry, so callers that ``except gnnstocks
... ValidationError`` keep working unchanged).
...
def probabilistic_sharpe_ratio(
    ...
    *,
    ...
    is the **full** (non-excess) kurtosis (Gaussian = ``3``); the excess-vs-full
    mix-up is a known footgun and is rejected. Returns the PSR in ``[0, 1]``.
...
def deflated_sharpe_ratio(
    ...
    *,
    ...
    count the FULL explored configuration grid (#graph-types x #models x #HP
    configs); ``variance_of_trial_sharpes`` (``V``) must be the REAL cross-trial
    variance of the per-observation trial Sharpes (use
    ``quantcore.variance_of_trial_sharpes``), never a hardcoded constant — with
    ``V == 0`` or ``N == 1`` the benchmark collapses to ``0`` and the DSR
    degenerates to the plain PSR (the multiplicity correction is silently
    disabled). Returns the DSR in ``[0, 1]``; non-increasing in ``n_trials``.
