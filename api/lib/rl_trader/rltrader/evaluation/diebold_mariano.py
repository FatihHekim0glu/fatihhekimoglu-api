"""Diebold-Mariano (1995) test on the RL-vs-buy-hold per-bar net-return differential.

The DM test compares two strategies' out-of-sample per-bar performance. With the
RL agent's per-bar net-return series ``r_rl_t`` and buy-and-hold's ``r_bh_t``, the
differential ``d_t = r_rl_t - r_bh_t`` has mean ``d_bar``; the DM statistic is
``d_bar / HAC_SE(d)``, asymptotically standard normal under the null of equal
performance. A POSITIVE statistic with a small p-value means the RL agent beats
...
better), so the RL agent beats buy-and-hold when the statistic is POSITIVE (a
...
:func:`rltrader.evaluation.metrics.hac_standard_error`.
...
from rltrader.evaluation.metrics import _coerce_pair, hac_standard_error

# quantcore-candidate: mirrors gnn-stocks:evaluation/diebold_mariano.py with the
# skill differential replaced by the per-bar net-RETURN differential (same sign:
# higher is better, so a positive statistic favours the RL agent).
...
def diebold_mariano(
    ...
    *,
    ...
    With per-bar net-return series ``net_returns_rl`` and
    ``net_returns_baseline``, the differential ``d_t = rl_t - baseline_t`` has mean
    ``d_bar``; the DM statistic is ``d_bar / HAC_SE(d)``, asymptotically standard
    normal under the null of equal performance. A POSITIVE statistic with a small
    p-value means the RL agent beats the baseline (a *higher* mean net return); a
    p-value ``>= alpha`` means the difference is insignificant (the honest NULL).

    Parameters
    ----------
    net_returns_rl:
        The RL agent's per-bar net-return series.
    ...
    tuple[float, float]
        ...
        agent; the p-value is clipped to ``[0, 1]``.
    ...
    # mean means the RL agent has the higher mean net return.
    diff = rl - baseline
    if diff.size < 2:
...
def dm_favours_model(dm_statistic: float, dm_pvalue: float, *, alpha: float = 0.05) -> bool:
    """Return ``True`` iff DM is significant AND signed in the RL agent's favour.

    The RL agent beats the baseline only when the two-sided p-value clears the
    ...
    the pure :func:`rltrader.evaluation.verdict.derive_verdict` gate.

    Parameters
    ----------
    dm_statistic:
        The Diebold-Mariano statistic (POSITIVE favours the RL agent).
    dm_pvalue:
