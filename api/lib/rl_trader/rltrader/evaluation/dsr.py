...
Probabilistic/Deflated Sharpe kernels here are byte-identical to quantcore's, so
this module re-exports them behind the original ``rltrader`` public names. The
only adaptation is the exception boundary — quantcore raises its OWN
``quantcore.ValidationError`` (no shared ancestry with rltrader's), so each call
is wrapped to translate it to :class:`rltrader._exceptions.ValidationError` with
the IDENTICAL message string, preserving every caller's ``except ValidationError``
...
def probabilistic_sharpe_ratio(
    ...
    *,
    ...
    try:
        ...
        # quantcore's ValidationError shares no ancestry with rltrader's; translate
...
def deflated_sharpe_ratio(
    ...
    *,
    ...
    try:
        ...
        # rltrader's with the IDENTICAL message string.
        raise ValidationError(str(exc)) from exc
