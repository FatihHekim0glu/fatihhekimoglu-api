...
The headline honesty mechanism. A PPO agent is trained across N INDEPENDENT seeds;
each seed produces its own OOS net Sharpe. The DISPERSION of those Sharpes — not
any single-seed equity curve — is the result. This module summarizes that
...
- the spread (min / max / IQR) of the seed Sharpes.
...
# quantcore-candidate: the across-seed bootstrap mirrors pairs-trading's block /
# percentile bootstrap, here resampling the per-SEED Sharpes (the training-path
# lottery) instead of per-bar returns.
...
def seed_lottery(
    ...
    *,
    ...
    with replacement via :func:`rltrader._rng.make_rng` (seeded, reproducible) and
    takes the ``alpha/2`` and ``1 - alpha/2`` percentiles of the resampled medians;
    with too few seeds it falls back to the empirical percentiles of the seed
    Sharpes themselves.
