"""Purged, embargoed walk-forward splits over a single-asset bar path.
...
OOS evaluation (no test-time learning / no online updates). The purge removes the
boundary bars whose look-back observation window or next-bar reward horizon would
straddle the train/test split, and the embargo holds out the return-horizon bars
after the train window so no train reward leaks into the OOS window.

This is the single-asset analogue of the portfolio walk-forward engine; the fold
geometry is pure integer arithmetic over the time axis (no price data peeked).
...
def required_purge(lookback: int, horizon: int = 1) -> int:
    ...
    between the train and test windows removes every boundary bar whose
    observation window or reward horizon would straddle the split (the no-leakage
    requirement).
...
def make_folds(
    ...
    *,
    ...
    Each fold trains on ``[0, test_start - purge)`` (anchored / expanding when
    ``anchored``) and is scored OOS on ``[test_start, test_end)``, where
    ...
    InsufficientDataError
        If the path is too short to host a single purged fold.
    """
    if n_folds < 1:
