...
public names so every call site and ``edgar_nlp``'s public API are unchanged. The
kernel is byte-identical (parity verified to 0.0 over thousands of random
inputs), so the migration is strictly behavior-preserving.
...
rest of ``edgar_nlp`` — and its test-suite ``pytest.raises(...)`` blocks — expect
:class:`edgar_nlp._exceptions.ValidationError` (an ``EdgarNlpError`` subclass).
The thin wrappers below translate the former to the latter with the IDENTICAL
message so the catch semantics are preserved.
...
# quantcore-candidate: MIGRATED — mirrors pairs-trading:evaluation/dsr.py
# (cross-checked to ma-crossover-backtest:data_snooping.py for the (k+2)/4 term).
...
def _norm_ppf(p: float) -> float:
    ...
    Thin wrapper that delegates to the shared quantcore kernel (Acklam's rational
    approximation with a Halley refinement step, numerically byte-identical to the
    former local implementation) and only translates a domain failure from
    ``quantcore``'s :class:`quantcore.ValidationError` to
    :class:`edgar_nlp._exceptions.ValidationError` so the existing catch semantics
    are preserved.
    """
    try:
...
def probabilistic_sharpe_ratio(
    ...
    *,
    ...
    failed precondition is surfaced as :class:`edgar_nlp._exceptions.ValidationError`
