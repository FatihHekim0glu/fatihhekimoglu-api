...
Drift-elimination: the PSR / DSR kernels are now thin re-exports of the shared
``quantcore`` package (``quantcore.probabilistic_sharpe_ratio`` /
``quantcore.deflated_sharpe_ratio``), which was seeded from this very repo's
implementation, so the math is byte-identical. The local public names are kept so
every call site and the package public API are unchanged; the only adaptation is
translating ``quantcore.ValidationError`` to this repo's
:class:`algosystem._exceptions.ValidationError` (with the IDENTICAL message
string) so ``pytest.raises(algosystem...ValidationError)`` call sites keep their
catch semantics. The private ``_norm_cdf`` / ``_norm_ppf`` helpers are re-exported
from quantcore too. Importing this module has no side effects.
"""
...
from quantcore.dsr import _norm_cdf, _norm_ppf  # noqa: F401  (re-exported helpers)

from algosystem._exceptions import ValidationError

# Euler-Mascheroni constant for the expected-maximum order statistic. Retained for
# any caller that imported it from this module historically.
